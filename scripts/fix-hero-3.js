...
const importIndex = code.indexOf(`import { motion }`);
if (importIndex === -1) {
...
// Remove original imports and inject the combined imports + function 
// we will replace from top to export function HeroSection
let finalCode = code.replace(/import \{ motion \} from "framer-motion";\nimport \{ ArrowRight, Zap, Target, Cpu \} from "lucide-react";\n\nexport function HeroSection\(\) \{/, 
componentCode + "\\nexport function HeroSection() {");

fs.writeFileSync("src/components/HeroSection.tsx", finalCode);
